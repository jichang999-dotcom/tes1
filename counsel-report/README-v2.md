...
| 데이터 정합성 | DB 원본 기준 (항상 최신) | **누적 파일 기준 (DB 수정 반영 안 됨)** |
| 과거 데이터 수정 반영 | 다음 실행 시 자동 반영 | **별도 보정 배치 필요** |
...
| 한계 | 설명 | 대응 방안 |
|------|------|-----------|
| **과거 데이터 수정 미반영** | DB에서 과거 상담 내용이 수정되어도 누적 파일에는 반영 안 됨 | 주 1회 보정 배치 (아래 참조) |
...
3. daily_entries에 오늘 날짜 키로 추가
   └─ 동일 날짜 키가 이미 있으면 → 덮어쓰기 (재실행/보정 대응)
...
과거 데이터 수정을 반영하기 위한 선택적 보정 배치.

```
node correction_batch.js
  → MariaDB에서 최근 7일 데이터 재조회
  → 누적 파일의 해당 일자와 비교
  → 변경된 건만 재분류 + 재분석
  → sentiment_accumulated.json 해당 일자 갱신
```
...
9. **과거 데이터 수정**: DB에서 과거 상담 내용이 수정되어도 누적 파일에 자동 반영 안 됨. 필요 시 `correction_batch.js` 실행.
